...
// Create DirectX Font
//=============================================================================
bool TextDX::initialize(ID3D10Device* g, int height, bool bold, bool italic, 
...
{                  // the graphics system
    ...
    // create DirectX font
    if(FAILED(D3DX10CreateFont(g, height, 0, weight, 1, italic,
...
int TextDX::print(const std::string &str, int x, int y)
{
    ...
    return dxFont->DrawTextA(NULL, str.c_str(), -1, &fontRect, DT_LEFT, color);
}
...
int TextDX::print(const std::string &str, RECT &rect, UINT format)
{
    ...
    // Tell the sprite about the matrix "Hello Neo"
    //graphics->getSprite()->SetTransform(&matrix);
    return dxFont->DrawTextA(NULL, str.c_str(), -1, &rect, format, color);
}
