...
LadderBillboard::LadderBillboard()
{
	ZeroMemory(this, sizeof(LadderBillboard));
}
...
void LadderBillboard::init(ID3D10Device* device, const D3DXVECTOR3 centers[], UINT numTrees)
{
	md3dDevice = device;
...
void LadderBillboard::draw(const Light& L, const D3DXVECTOR3& eyePosW, const D3DXMATRIX& viewProj)
{
...
void LadderBillboard::buildVB(const D3DXVECTOR3 centers[])
{
	...
	for(UINT i = 0; i < mNumTrees; ++i)
	{
		...
		v[i].sizeW    = D3DXVECTOR2(96.0f, 800.0f);
	}
...
void LadderBillboard::buildFX()
{
	...
	HRESULT hr = 0;
	hr = D3DX10CreateEffectFromFile(L"source\\tree.fx", 0, 0, 
...
void LadderBillboard::buildVertexLayout()
{
...
void LadderBillboard::buildShaderResourceView()
{
	//
