...
Billboard::Billboard()
{
	ZeroMemory(this, sizeof(Billboard));
}
...
void Billboard::init(ID3D10Device* device, const D3DXVECTOR3 centers[], UINT numTrees, std::wstring fn, float _width, float _height)
{
	width = _width;
	height = _height;
	filename = fn;
...
void Billboard::draw(const Light& L, const D3DXVECTOR3& eyePosW, const D3DXMATRIX& viewProj)
{
	if (!active) return;
...
void Billboard::buildVertexLayout()
{
...
void Billboard::buildShaderResourceView()
{
	//
